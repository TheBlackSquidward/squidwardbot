...
public class BotInfoCommand extends SquidwardBotCommand {
  ...
  public void onSlashCommand(SlashCommandInteractionEvent event) {
    ...
    event
        .getHook()
        .sendMessageEmbeds(
            createBotInfoEmbed(event.getJDA(), event.isFromGuild() ? event.getGuild() : null))
        .queue();
  }
  ...
  private MessageEmbed createBotInfoEmbed(JDA jda, @Nullable Guild guild) {
    ...
    embedBuilder.setColor(
        isFromGuild ? guild.getSelfMember().getColor() : ColorConstants.PRIMARY_COLOR);
    embedBuilder.addField(
        "Name",
        isFromGuild ? guild.getSelfMember().getEffectiveName() : jda.getSelfUser().getName(),
        false);
    embedBuilder.addField(
        "Created On", StringUtils.formatTime(jda.getSelfUser().getTimeCreated()), false);
    if (isFromGuild) {
      embedBuilder.addField(
          "Joined At", StringUtils.formatTime(guild.getSelfMember().getTimeJoined()), false);
    }
    ...
    embedBuilder.addField(
        "Channels",
        String.valueOf(
            jda.getVoiceChannelCache().size()
                + jda.getTextChannelCache().size()
                + jda.getThreadChannelCache().size()),
        true);
    embedBuilder.addField(
        "Slash Commands", String.valueOf(CommandRegistry.getCommandsSize()), true);
    embedBuilder.addField(
        "Bot Owner", jda.getUserById(Environment.getInstance().getOwnerId()).getAsMention(), false);
