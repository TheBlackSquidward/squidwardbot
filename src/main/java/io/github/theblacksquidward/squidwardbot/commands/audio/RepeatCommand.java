  ...
  public void onSlashCommand(SlashCommandInteractionEvent event) {
    ...
    if (!event.getGuild().getAudioManager().isConnected()) {
      event
          .getHook()
          .sendMessageEmbeds(
              createMusicReply("The bot must be connected to a voice channel to repeat a track."))
          .queue();
      return;
    }
    if (event.getMember().getVoiceState().getChannel().getIdLong() != audioChannel.getIdLong()) {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "You must be in the same voice channel as the bot to repeat a track."))
          .queue();
      return;
    }
    if (AudioManager.getCurrentlyPlayingTrack(guild) == null) {
      event
          .getHook()
          .sendMessageEmbeds(
              createMusicReply("Could not repeat as there is no currently playing track."))
          .queue();
      return;
    }
    ...
    event
        ...
        .sendMessageEmbeds(
            createMusicReply(
                "Repeating has been toggled to `" + AudioManager.isRepeating(guild) + "`."))
        .queue();
  }
