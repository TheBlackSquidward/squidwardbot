...
public class NowPlayingCommand extends AbstractAudioCommand {
  ...
  public void onSlashCommand(SlashCommandInteractionEvent event) {
    ...
    if (!event.getGuild().getAudioManager().isConnected()) {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "The bot must be connected to a voice channel to get the currently playing track."))
          .queue();
      return;
    }
    ...
    if (currentTrack == null) {
      event
          .getHook()
          .sendMessageEmbeds(createMusicReply("There is no song currently playing..."))
          .queue();
      return;
    }
  ...
  private String getProgress(AudioTrack nowPlaying) {
    final int percentage =
        Math.round((float) nowPlaying.getPosition() / nowPlaying.getDuration() * 100);
    return "[**"
        + StringUtils.millisecondsFormatted(nowPlaying.getPosition())
        + "**/**"
        + StringUtils.millisecondsFormatted(nowPlaying.getDuration())
        + "**] "
        + createProgressBar(nowPlaying.getDuration(), nowPlaying.getPosition())
        + " ("
        + percentage
        + "%)";
  }
  ...
  private String createProgressBar(long total, long current) {
    ...
    final int progress =
        (int) Math.max(0, Math.min(Math.round(arraySize * percentage), arraySize - 1));
