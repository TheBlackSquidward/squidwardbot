...
public class VolumeCommand extends AbstractAudioCommand {
    ...
    public void onSlashCommand(SlashCommandInteractionEvent event) {
        ...
        if(event.getSubcommandName().equalsIgnoreCase("get")) {
            event.getHook().sendMessageEmbeds(createMusicReply("The current volume of the bot is `" + AudioManager.getVolume(guild) + "`.")).queue();
            return;
        }
        if(event.getSubcommandName().equalsIgnoreCase("reset")) {
            event.getHook().sendMessageEmbeds(createMusicReply("The volume of the audio player has been reset to the default value of `100`.")).queue();
        ...
        if(event.getSubcommandName().equalsIgnoreCase("set")) {
            ...
            event.getHook().sendMessageEmbeds(createMusicReply("The volume has successfully been set to `" + AudioManager.getVolume(guild) + "`.")).queue();
        }
    }
