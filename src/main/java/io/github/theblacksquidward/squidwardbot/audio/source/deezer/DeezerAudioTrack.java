...
public class DeezerAudioTrack extends DelegatedAudioTrack {
  ...
  private URI getTrackMediaURI() throws IOException, URISyntaxException {
    HttpPost getSessionID =
        new HttpPost(
            DeezerAudioSourceManager.PRIVATE_API_BASE
                + "?method=deezer.ping&input=3&api_version=1.0&api_token=");
    JsonBrowser json =
        HttpClientTools.fetchResponseAsJson(sourceManager.getHttpInterface(), getSessionID);
    ...
    getMediaURL.setEntity(
        new StringEntity(
            "{\"license_token\":\""
                + userLicenseToken
                + "\",\"media\": [{\"type\": \"FULL\",\"formats\": [{\"cipher\": \"BF_CBC_STRIPE\", \"format\": \"MP3_128\"}]}],\"track_tokens\": [\""
                + trackToken
                + "\"]}",
    ...
    return new URI(
        json.get("data").index(0).get("media").index(0).get("sources").index(0).get("url").text());
  }

  private byte[] getTrackDecryptionKey() throws NoSuchAlgorithmException {
    char[] md5 =
        Hex.encodeHex(
            MessageDigest.getInstance("MD5").digest(this.trackInfo.identifier.getBytes()), true);
  ...
  public void process(LocalAudioTrackExecutor executor) throws Exception {
    try (HttpInterface httpInterface = sourceManager.getHttpInterface()) {
      try (DeezerPersistentHttpStream stream =
          new DeezerPersistentHttpStream(
              httpInterface, getTrackMediaURI(), trackInfo.length, getTrackDecryptionKey())) {
