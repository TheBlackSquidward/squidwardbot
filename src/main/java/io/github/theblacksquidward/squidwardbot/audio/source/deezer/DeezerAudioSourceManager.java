...
public class DeezerAudioSourceManager implements AudioSourceManager, HttpConfigurable {
  ...
  public static final Pattern URL_PATTERN =
      Pattern.compile(
          "(https?://)?(www\\.)?deezer\\.com/(?<countrycode>[a-zA-Z]{2}/)?(?<type>track|album|playlist|artist)/(?<identifier>[0-9]+)");
  ...
  public AudioItem loadItem(AudioPlayerManager manager, AudioReference reference) {
    try {
      ...
      // If the identifier is a share URL, we need to follow the redirect to find out the real url
      // behind it
      if (reference.identifier.startsWith(SHARE_URL)) {
        HttpGet request = new HttpGet(reference.identifier);
        request.setConfig(RequestConfig.custom().setRedirectsEnabled(false).build());
        try (CloseableHttpResponse response =
            this.httpInterfaceManager.getInterface().execute(request)) {
  ...
  private List<AudioTrack> getSearchResults(String query) throws IOException {
    JsonBrowser json =
        getJson(PUBLIC_API_BASE + "/search?q=" + URLEncoder.encode(query, StandardCharsets.UTF_8));
  ...
  public AudioItem getAlbum(String identifier) throws IOException {
    ...
    return json == null || json.get("tracks").get("data").values().isEmpty()
        ? AudioReference.NO_TRACK
        : new BasicAudioPlaylist(
            // TODO
    ...
    return json == null || json.get("data").values().isEmpty()
        ...
        : new BasicAudioPlaylist(
            ...
            new AudioPlaylistInfo(
                json.get("data").index(0).get("artist").get("name").text() + "'s Top Tracks",
                null,
                null,
                null,
                null),
