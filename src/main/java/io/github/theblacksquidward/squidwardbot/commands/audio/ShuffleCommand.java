  ...
  public void onSlashCommand(SlashCommandInteractionEvent event) {
    ...
    if (!event.getGuild().getAudioManager().isConnected()) {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "The bot must be connected to a voice channel to shuffle the queue."))
          .queue();
      return;
    }
    if (event.getMember().getVoiceState().getChannel().getIdLong() != audioChannel.getIdLong()) {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "You must be in the same voice channel as the bot to shuffle the queue."))
          .queue();
      return;
    }
    if (AudioManager.isRepeating(guild)) {
      event
          .getHook()
          .sendMessageEmbeds(createMusicReply("You cannot shuffle the repeating queue."))
          .queue();
    } else {
      if (AudioManager.getQueue(guild).isEmpty()) {
        event
            .getHook()
            .sendMessageEmbeds(
                createMusicReply("Could not shuffle the queue as it is currently empty."))
            .queue();
        return;
      }
      ...
      event
          .getHook()
          .sendMessageEmbeds(createMusicReply("Successfully shuffled the queue."))
          .queue();
    }
  }
