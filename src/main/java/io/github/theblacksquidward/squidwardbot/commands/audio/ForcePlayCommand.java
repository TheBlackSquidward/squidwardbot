...
public class ForcePlayCommand extends AbstractAudioCommand {
  ...
  public void onSlashCommand(SlashCommandInteractionEvent event) {
    ...
    if (event.getMember().getVoiceState().getChannel().getIdLong() != audioChannel.getIdLong()) {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "You must be in the same voice channel as the bot to force play an audio track."))
          .queue();
      return;
    }
  ...
  public List<OptionData> getOptionData() {
    return List.of(
        new OptionData(
            OptionType.STRING, "identifier", "Identifier of the track (URL/Name)", true));
  }
  ...
  private static class AudioLoadResultImpl extends BaseAudioLoadResultImpl {
    ...
    public void trackLoaded(AudioTrack audioTrack) {
      ...
      embedBuilder.setDescription(
          "Successfully force loaded the track "
