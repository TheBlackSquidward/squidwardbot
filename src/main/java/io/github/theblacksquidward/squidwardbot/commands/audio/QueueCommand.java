...
public class QueueCommand extends AbstractAudioCommand {
  ...
  public void onSlashCommand(SlashCommandInteractionEvent event) {
    ...
    if (!event.getGuild().getAudioManager().isConnected()) {
      event
          .getHook()
          .sendMessageEmbeds(
              createMusicReply("The bot must be connected to a voice channel to view the queue."))
          .queue();
      return;
    }
    if (event.getMember().getVoiceState().getChannel().getIdLong() != audioChannel.getIdLong()) {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "You must be in the same voice channel as the bot to view the queue."))
          .queue();
      return;
    }
    if (AudioManager.isRepeating(guild)) {
      if (AudioManager.getRepeatingQueue(guild).isEmpty()) {
        event
            .getHook()
            .sendMessageEmbeds(
                createMusicReply("Could not view the repeating queue as it is currently empty."))
            .queue();
        return;
      }
      ...
      if (AudioManager.getQueue(guild).isEmpty()) {
        event
            .getHook()
            .sendMessageEmbeds(
                createMusicReply("Could not view the queue as it is currently empty."))
            .queue();
        return;
      }
      // TODO redo with pagination
  ...
  private MessageEmbed getRepeatingQueueEmbed(Guild guild) {
    return new EmbedBuilder()
        ...
        .setAuthor(
            "|  " + "Current Repeating Queue:",
            null,
            "https://avatars.githubusercontent.com/u/65785034?v=4")
  ...
  private String getRepeatingQueueAsString(Guild guild) {
    ...
    if (tracks.size() > trackCount) {
      stringBuilder.append("And `").append(tracks.size() - trackCount).append("` more...");
    }
  ...
  private MessageEmbed getQueueEmbed(Guild guild) {
    return new EmbedBuilder()
        .setColor(ColorConstants.PRIMARY_COLOR)
        .setAuthor(
            "|  " + "Current Queue:", null, "https://avatars.githubusercontent.com/u/65785034?v=4")
