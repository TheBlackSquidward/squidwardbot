  ...
  public void onButtonInteraction(@NotNull ButtonInteractionEvent event) {
    ...
    if (instruction.equalsIgnoreCase("next")) {
      ...
      final List<String> requestedLyrics =
          getLyricsPage(lyrics.lines().collect(Collectors.toList()), newPage);
      ...
      previous =
          previous
              .withId(
                  buttonId + "_" + channelId + "_" + userId + "_" + hitId + "_" + newPage + "_prev")
              .withDisabled(false);
      close =
          close
              .withId(
                  buttonId + "_" + channelId + "_" + userId + "_" + hitId + "_" + newPage
                      + "_close")
              .withDisabled(false);
      next =
          next.withId(
                  buttonId + "_" + channelId + "_" + userId + "_" + hitId + "_" + newPage + "_next")
              .withDisabled(false);
      event
          .editMessageEmbeds(
              new EmbedBuilder(event.getMessage().getEmbeds().get(0))
                  .setDescription(nextPage)
                  .build())
          .setComponents(ActionRow.of(previous, close, next))
          .queue();
    }
  ...
  public void onSlashCommand(SlashCommandInteractionEvent event) {
    ...
    try {
      ...
      if (hits.isEmpty()) {
        event
            .getHook()
            .sendMessageEmbeds(createMusicReply("There are no lyric results for this song."))
            .queue();
        return;
      }
      ...
      event
          .getHook()
          .sendMessageEmbeds(getLyricsEmbed(hit))
          .setComponents(
              ActionRow.of(
                  Button.primary(
                          "lyrics_"
