...
public class SquidwardBotDatabase {
    ...
    public static SquidwardBotDatabase getInstance() {
        if (INSTANCE == null)
            throw new NullPointerException("You cannot access the database without initializing it first!");
        return INSTANCE;
    }
    ...
    private MongoClient connect(CodecRegistry codecRegistry) {
        final ConnectionString connectionString = new ConnectionString(
                "mongodb+srv://" + Environment.getInstance().getMongoDBUsername() + ":" + Environment.getInstance().getMongoDBPassword()
                        + "@squidwardbot.nd0ncel.mongodb.net/?retryWrites=true&w=majority");
        final MongoClientSettings mongoClientSettings = MongoClientSettings.builder()
                .applyConnectionString(connectionString).applicationName("SquidwardBot").codecRegistry(codecRegistry).build();
