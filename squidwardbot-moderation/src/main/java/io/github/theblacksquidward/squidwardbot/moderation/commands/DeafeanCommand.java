...
public class DeafeanCommand extends SquidwardBotCommand {
    ...
    public void onSlashCommand(SlashCommandInteractionEvent event) {
        ...
        if(!member.getVoiceState().inAudioChannel()) {
            event.getHook().sendMessage("You cannot deafen someone who is not currently in a voice channel.").queue();
        }
        member.deafen(true).queue();
        event.getHook().sendMessage("You have successfully deafened " + member.getAsMention()).setEphemeral(true).queue();
    }
