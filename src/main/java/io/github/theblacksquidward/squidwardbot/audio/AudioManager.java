...
public class AudioManager {
  ...
  public static void init() {
    AUDIO_PLAYER_MANAGER.registerSourceManager(
        new SpotifySourceManager(
            new String[] {"dzsearch:%QUERY%", "dzisrc:%ISRC%"},
            Environment.getInstance().getSpotifyClientId(),
            Environment.getInstance().getSpotifyClientSecret(),
            "US",
            AUDIO_PLAYER_MANAGER));
    AUDIO_PLAYER_MANAGER.registerSourceManager(SoundCloudAudioSourceManager.createDefault());
    AUDIO_PLAYER_MANAGER.registerSourceManager(
        new DeezerAudioSourceManager(Environment.getInstance().getDeezerMasterDecryptionKey()));
  ...
  public static GuildAudioManager getOrCreate(@NotNull Guild guild) {
    return GUILD_AUDIO_MANAGERS.computeIfAbsent(
        guild.getIdLong(),
        guildId -> {
          final GuildAudioManager guildAudioManager =
              new GuildAudioManager(AUDIO_PLAYER_MANAGER.createPlayer());
  ...
  public static void requeueTrack(@NotNull Guild guild) {
    getOrCreate(guild)
        .getTrackScheduler()
        .addTrackAtHead(getCurrentlyPlayingTrack(guild).makeClone());
  }
  ...
  public static SongSearch getLyrics(@NotNull Guild guild, @NotNull AudioTrack audioTrack)
      throws IOException {
    return Constants.GENIUS_LYRICS_API.search(
        audioTrack.getInfo().title + "" + audioTrack.getInfo().author);
  }
  ...
  public static void loadAndPlay(@NotNull Guild guild, String identifier) {
    ...
    loadAndPlay(
        guild, identifier, new BaseAudioLoadResultImpl(guildAudioManager.getTrackScheduler()));
  }

  public static void loadAndPlay(
      @NotNull Guild guild, String identifier, AudioLoadResultHandler audioLoadResult) {
    final GuildAudioManager guildAudioManager = getOrCreate(guild);
