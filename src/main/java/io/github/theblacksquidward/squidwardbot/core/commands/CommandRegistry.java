  ...
  public static void captureAndRegisterCommands(Reflections reflections) {
    ...
    LOGGER.info(
        "Successfully found "
            ...
            + " commands, Attempting to register them...");
    annotatedClasses.forEach(
        (annotatedClass) -> {
          try {
            SquidwardBotCommand squidwardBotCommand =
                (SquidwardBotCommand) annotatedClass.getDeclaredConstructor().newInstance();
    ...
    timer.stop();
    LOGGER.info(
        "Finished capturing and registering commands in {}. Successfully registered {} commands.",
        timer,
        getCommands().size());
  }
    ...
    event
        ...
        .forEach(
            guild -> {
              ...
              getCommands()
                  .forEach(cmd -> commandListUpdateAction.addCommands(cmd.getCommandData()));
