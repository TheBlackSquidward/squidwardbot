...
public class UnpauseCommand extends AbstractAudioCommand {
    ...
    public void onSlashCommand(SlashCommandInteractionEvent event) {
        ...
        if(!event.getGuild().getAudioManager().isConnected()) {
            event.getHook().sendMessageEmbeds(createMusicReply("The bot must be connected to a voice channel to unpause the queue.")).queue();
            return;
        }
        if(event.getMember().getVoiceState().getChannel().getIdLong() != audioChannel.getIdLong()) {
            event.getHook().sendMessageEmbeds(createMusicReply("You must be in the same voice channel as the bot to unpause the queue.")).queue();
            return;
        }
        ...
        if(!AudioManager.isPaused(guild)) {
           event.getHook().sendMessageEmbeds(createMusicReply("The player is not currently paused...")).queue();
           return;
        }
        AudioManager.resumeTrack(guild);
        event.getHook().sendMessageEmbeds(createMusicReply("Successfully unpaused the player.")).queue();
    }
