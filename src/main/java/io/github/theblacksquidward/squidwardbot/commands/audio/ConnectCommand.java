...
public class ConnectCommand extends AbstractAudioCommand {
  ...
  public void onSlashCommand(SlashCommandInteractionEvent event) {
    ...
    if (event.getGuild().getAudioManager().isConnected()) {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "The bot is already connected to the channel"
    ...
    if (option == null) {
      if (guild.getMember(user) == null) {
        event
            ...
            .sendMessageEmbeds(
                createMusicReply(
                    "You are not currently in a voice channel. Either join one or specify one."))
            .queue();
        return;
      }
      ...
      if (channel.getType() != ChannelType.VOICE) {
        event
            .getHook()
            .sendMessageEmbeds(createMusicReply("The channel specified is not a voice channel."))
            .queue();
        return;
      }
      ...
      event
          .getHook()
          .sendMessageEmbeds(
              createMusicReply("Successfully connected to " + voiceChannel.getName()))
          .queue();
    }
  }
  ...
  public List<OptionData> getOptionData() {
    return List.of(
        new OptionData(
            OptionType.CHANNEL, "channel", "The channel the bot should be connected to", false));
  }
}
