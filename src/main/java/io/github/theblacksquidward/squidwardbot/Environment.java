...
public class Environment {
  ...
  public void loadDotenv(String[] args) {
    if (this.env != null) throw new IllegalStateException("Environment already loaded!");
    LOGGER.info("Loading environment...");
    final Stopwatch timer = Stopwatch.createStarted();
    OptionParser optionParser = new OptionParser();
    OptionSpec<String> envFilePathOptionSpec =
        optionParser.accepts("env").withRequiredArg().required().ofType(String.class);
    OptionSet optionSet = optionParser.parse(args);
    String filePath = envFilePathOptionSpec.value(optionSet);
    this.env = Dotenv.configure().directory(filePath).load();
    LOGGER.debug(
        "Loaded environment variables: {}",
        StringUtils.getIndentedStringList(getEnvironmentKeys()));
    timer.stop();
    LOGGER.info(
        "Successfully loaded environment in {}. Successfully loaded {} environment variables.",
  ...
  public Set<String> getEnvironmentKeys() {
    return env.entries(Dotenv.Filter.DECLARED_IN_ENV_FILE).stream()
