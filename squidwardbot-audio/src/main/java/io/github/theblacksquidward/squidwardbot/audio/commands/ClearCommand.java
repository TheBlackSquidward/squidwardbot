...
public class ClearCommand extends AbstractAudioCommand {
    ...
    public void onSlashCommand(SlashCommandInteractionEvent event) {
        ...
        if(!event.getGuild().getAudioManager().isConnected()) {
            event.replyEmbeds(createMusicReply("The bot must be connected to a voice channel to clear the queue.")).queue();
            return;
        }
        if(event.getMember().getVoiceState().getChannel().getIdLong() != audioChannel.getIdLong()) {
            event.replyEmbeds(createMusicReply("You must be in the same voice channel as the bot to clear the queue.")).queue();
            return;
        }
        if(AudioManager.isQueueEmpty(guild)) {
            event.replyEmbeds(createMusicReply("The queue is currently empty... There is nothing to skip.")).queue();
            return;
        }
        AudioManager.clearQueue(guild);
        event.replyEmbeds(createMusicReply("Successfully cleared the queue.")).queue();
    }
