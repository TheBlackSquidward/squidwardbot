...
public class RemoveCommand extends AbstractAudioCommand {
    ...
    if (AudioManager.isRepeating(guild)) {
      ...
      if (position > repeatingQueue.size() || position < 1) {
        event
            .getHook()
            .sendMessageEmbeds(
                createMusicReply("You cannot remove a track that doesn't exist in the queue."))
            .queue();
        return;
      }
      ...
      event
          .getHook()
          .sendMessageEmbeds(
              createMusicReply("Successfully removed track at position " + position + "."))
          .queue();
    } else {
  ...
  public List<OptionData> getOptionData() {
    return List.of(
        new OptionData(
            OptionType.INTEGER, "position", "The position of the track to be removed", true));
  }
}
