...
public class KissCommand extends AbstractGifCommand {
    ...
    public void onSlashCommand(SlashCommandInteractionEvent event) {
        ...
        if(giver == receiver) {
            MessageEmbed embed = getLonelyEmbedBuilder()
                    ...
                    .setDescription("Awwwwww is someone lonely " + giver.getAsMention() + ". You cannot kiss yourself.")
                    .build();
            event.replyEmbeds(embed).queue();
            return;
        }
        MessageEmbed embed = getGifEmbedBuilder("anime kiss")
    ...
    public java.util.List<OptionData> getOptionData() {
        return List.of(
                new OptionData(OptionType.USER, "user", "Your kissing buddy!", true)
        );
    }
