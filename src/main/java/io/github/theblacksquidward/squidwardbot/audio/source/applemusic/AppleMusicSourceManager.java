...
public class AppleMusicSourceManager extends MirroringAudioSourceManager
  ...
  public static final Pattern URL_PATTERN =
      Pattern.compile(
          "(https?://)?(www\\.)?music\\.apple\\.com/(?<countrycode>[a-zA-Z]{2}/)?(?<type>album|playlist|artist|song)(/[a-zA-Z\\d\\-]+)?/(?<identifier>[a-zA-Z\\d\\-.]+)(\\?i=(?<identifier2>\\d+))?");
  public static final Pattern TOKEN_SCRIPT_PATTERN =
      Pattern.compile("const \\w{2}=\"(?<token>(ey[\\w-]+)\\.([\\w-]+)\\.([\\w-]+))\"");
  ...
  public void parseTokenData() throws IOException {
    ...
    JsonBrowser json =
        JsonBrowser.parse(new String(Base64.getDecoder().decode(mediaAPIToken.split("\\.")[1])));
  ...
  public void requestToken() throws IOException {
    ...
    try (CloseableHttpResponse response = httpInterfaceManager.getInterface().execute(request)) {
      ...
      for (Element element : elements) {
        ...
        try (CloseableHttpResponse indexResponse =
            httpInterfaceManager.getInterface().execute(request)) {
          String tokenScript =
              IOUtils.toString(indexResponse.getEntity().getContent(), StandardCharsets.UTF_8);
  ...
  public String getMediaAPIToken() throws IOException {
    if (mediaAPIToken == null || tokenExpire == null || tokenExpire.isBefore(Instant.now()))
      requestToken();
    ...
    return searchResults.isEmpty()
        ...
        : new BasicAudioPlaylist(
            ...
            new AudioPlaylistInfo(
                "Apple Music Search Results For: " + query, null, null, null, null),
            searchResults,
            null,
            true);
  }
    ...
    JsonBrowser json =
        getJson(
            API_BASE
                ...
                + "&limit="
                + 25);
    return json == null || json.get("results").get("songs").get("data").values().isEmpty()
        ? Collections.emptyList()
        : parseTracks(json.get("results").get("songs"));
  }
  ...
  public AudioItem getArtist(String identifier, String countryCode) throws IOException {
    JsonBrowser json =
        getJson(API_BASE + "catalog/" + countryCode + "/artists/" + identifier + "/view/top-songs");
    return json == null || json.get("data").values().isEmpty()
        ? AudioReference.NO_TRACK
        : new BasicAudioPlaylist(
            // TODO
            new AudioPlaylistInfo(
                json.get("data").index(0).get("attributes").get("artistName").text()
                    + "'s Top Tracks",
                null,
                null,
                null,
  ...
  private List<AudioTrack> parseTracks(JsonBrowser json) {
    return json.get("data").values().stream().map(this::parseTrack).collect(Collectors.toList());
  }
