...
public class PlayCommand extends AbstractAudioCommand {
            ...
            .filter(
                json ->
                    !json.get("title")
                        ...
                        .get("text")
                        .text()
                        .equalsIgnoreCase("Community playlists"))
            .forEach(
                json ->
                    json.get("contents")
                        .values()
                        .forEach(
                            json1 -> {
                              JsonBrowser musicResponsiveListItemRenderer =
                                  json1.get("musicResponsiveListItemRenderer");
                              List<JsonBrowser> flexColumns =
                                  musicResponsiveListItemRenderer.get("flexColumns").values();
                              JsonBrowser firstColumn = flexColumns.get(0);
                              List<JsonBrowser> metadata =
                                  flexColumns
                                      .get(1)
                              ...
                              if (videoId != null)
                                value = YoutubeConstants.WATCH_URL_PREFIX + videoId;
                              else if (browseId != null && browseId.startsWith("UC"))
                                value = YoutubeConstants.MUSIC_CHANNEL_URL_PREFIX + browseId;
                              else if (playlistId != null)
                                value = YoutubeConstants.MUSIC_PLAYLIST_URL_PREFIX + playlistId;

                              // TODO: Implement a way to send all give choices irrespective of
                              // size.
                              if (name.length() < 100)
                                choices.add(
  ...
  public void onSlashCommand(SlashCommandInteractionEvent event) {
    ...
    if (!event.getGuild().getAudioManager().isConnected()) {
      ...
      event
          .getHook()
          .sendMessageEmbeds(
              createMusicReply("Successfully connected to " + audioChannel.getName()))
          .queue();
    }
    if (event.getMember().getVoiceState().getChannel().getIdLong() != audioChannel.getIdLong()) {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "You must be in the same voice channel as the bot to add an audio track to the queue."))
          .queue();
      return;
    }
    ...
    AudioManager.loadAndPlay(
        guild,
        identifier,
        new AudioLoadResultImpl(
            event, identifier, AudioManager.getOrCreate(guild).getTrackScheduler()));
  }
  ...
  public List<OptionData> getOptionData() {
    return List.of(
        new OptionData(
            OptionType.STRING, QUERY_ID, "Identifier of the track (URL/Name)", true, true));
  }
  ...
  private static class AudioLoadResultImpl extends BaseAudioLoadResultImpl {
    ...
    public AudioLoadResultImpl(
        SlashCommandInteractionEvent event, String identifier, TrackScheduler trackScheduler) {
    ...
    public void trackLoaded(AudioTrack audioTrack) {
      ...
      embedBuilder.setDescription(
          "Successfully queued the track "
              + audioTrackInfo.title
              + " by "
              + audioTrackInfo.author
              + " ["
              + StringUtils.millisecondsFormatted(audioTrack.getDuration())
              + "] at position #"
              + AudioManager.getPositionInQueue(event.getGuild(), audioTrack)
              + " in the queue.");
    ...
    public void playlistLoaded(AudioPlaylist audioPlaylist) {
      ...
      EmbedBuilder embedBuilder = new EmbedBuilder();
      List<String> formattedPlaylistTracks =
    ...
    public void noMatches() {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "Could not match the given identifier: `"
                      + identifier
                      + "` to an audio track or an audio playlist."))
          .queue();
      super.noMatches();
    }
