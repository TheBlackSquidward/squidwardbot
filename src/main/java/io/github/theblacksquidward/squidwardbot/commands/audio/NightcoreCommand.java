  ...
  public void onSlashCommand(SlashCommandInteractionEvent event) {
    ...
    if (!event.getGuild().getAudioManager().isConnected()) {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "The bot must be connected to a voice channel to apply the nightcore effect to the audio player."))
          .queue();
      return;
    }
    if (event.getMember().getVoiceState().getChannel().getIdLong() != audioChannel.getIdLong()) {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "You must be in the same voice channel as the bot to apply the nightcore effect to the audio player."))
          .queue();
      return;
    }
    ...
    if (AudioManager.isSlowed(guild)) {
      event
          ...
          .sendMessageEmbeds(
              createMusicReply(
                  "The nightcore effect cannot be applied to the audio player as it currently has the slow effect."))
          .queue();
      return;
    }
    if (AudioManager.isNightcore(guild)) {
      event
          .getHook()
          .sendMessageEmbeds(
              createMusicReply("The nightcore effect has been removed from the player."))
          .queue();
    ...
    event
        .getHook()
        .sendMessageEmbeds(createMusicReply("The nightcore effect has been applied to the player."))
        .queue();
