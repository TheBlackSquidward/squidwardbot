...
public class BassBoostCommand extends AbstractAudioCommand {
  ...
  public void onSlashCommand(SlashCommandInteractionEvent event) {
    ...
    if (!event.getMember().getVoiceState().inAudioChannel()) {
      event
          .deferReply()
          .addEmbeds(createMusicReply("You must be in a voice channel to use this command."))
          .queue();
      return;
    }
    ...
    if (!event.getGuild().getAudioManager().isConnected()) {
      event
          ...
          .addEmbeds(
              createMusicReply(
                  "The bot must be connected to a voice channel to bass boost a song."))
          .queue();
      return;
    }
    if (event.getMember().getVoiceState().getChannel().getIdLong() != audioChannel.getIdLong()) {
      event
          ...
          .addEmbeds(
              createMusicReply(
                  "You must be in the same voice channel as the bot to bass boost a song."))
          .queue();
      return;
    }
    if (!AudioManager.isPlayingTrack(guild)) {
      event
          .deferReply()
          .addEmbeds(createMusicReply("The bot is not currently playing anything..."))
          .queue();
      return;
    }
    if (event.getSubcommandName().equalsIgnoreCase("enable")) {
      if (AudioManager.isBassBoosted(guild)) {
        event
            .deferReply()
            .addEmbeds(createMusicReply("The audio player is already bass boosted."))
            .queue();
        return;
      }
      ...
      event
          .deferReply()
          .addEmbeds(createMusicReply("Bass boost has been enabled for this audio player."))
          .queue();
    }
    if (event.getSubcommandName().equalsIgnoreCase("disable")) {
      if (!AudioManager.isBassBoosted(guild)) {
        event
            .deferReply()
            .addEmbeds(createMusicReply("The audio player is not currently bass boosted."))
            .queue();
        return;
      }
      ...
      event
          .deferReply()
          .addEmbeds(createMusicReply("Bass boost has been disabled for this audio player."))
          .queue();
    }
    if (event.getSubcommandName().equalsIgnoreCase("get")) {
      ...
      event
          ...
          .addEmbeds(
              createMusicReply(
                  "The current bass boost percentage is "
                      + AudioManager.getBassBoostPercentage(guild)
                      + "%."))
          .queue();
    }
    if (event.getSubcommandName().equalsIgnoreCase("set")) {
      ...
      event
          ...
          .addEmbeds(
              createMusicReply(
                  "The bass boost multiplier has been set to "
                      + bassBoostMultiplier.getAsInt()
                      + "%."))
          .queue();
    }
  }
  ...
  public List<SubcommandData> getSubcommandData() {
    return List.of(
        ...
        new SubcommandData("set", "Sets the bass boost level.")
            .addOptions(
                new OptionData(
                        OptionType.INTEGER, "bassboost", "The percentage bass boost level.", true)
