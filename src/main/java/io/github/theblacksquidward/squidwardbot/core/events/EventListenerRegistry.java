...
public class EventListenerRegistry {
    ...
    public static void captureAndRegisterEventListeners(Reflections reflections) {
        ...
        LOGGER.info("Successfully found " + annotatedClasses.size() + " event listeners, Attempting to register them...");
        annotatedClasses.forEach((annotatedClass) -> {
            try {
                SquidwardBotEventListener squidwardBotEventListener = (SquidwardBotEventListener) annotatedClass.getDeclaredConstructor().newInstance();
        ...
        LOGGER.debug("Loaded event listeners: {}", StringUtils.getIndentedStringList(getEventListeners()));
        timer.stop();
        LOGGER.info("Finished capturing and registering event listeners in {}. Successfully registered {} event listeners.", timer, getEventListenersSize());
    }
